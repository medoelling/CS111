...
#define TASKBUFSIZ	65536 //4096	// Size of task_t::buf
...
static size_t get_offset(int blockno)
{
  return blockno * BLKSIZE;
}

static peer_t *get_peer(peer_t* list, int i)
{
  int j;
  peer_t *ret = list;
  for(j = 0; j < i; j++){
...
taskbufresult_t read_to_taskbuf(int fd, task_t *t)
{
	...
	if (t->head == t->tail || headpos < tailpos){
	  nbytes = (BLKSIZE > TASKBUFSIZ - tailpos) ? TASKBUFSIZ - tailpos : BLKSIZE;
		amt = read(fd, &t->buf[tailpos], nbytes);
	}
	else{
	  	  nbytes = (BLKSIZE > headpos - tailpos) ? headpos - tailpos : BLKSIZE;
		amt = read(fd, &t->buf[tailpos], nbytes);
	}
...
taskbufresult_t write_from_taskbuf(int fd, task_t *t)
{
	...
	if (t->head == t->tail)
	  ...
	  nbytes = (BLKSIZE > tailpos - headpos) ?  tailpos - headpos : BLKSIZE;
		amt = write(fd, &t->buf[headpos], nbytes);
	}
	else{
	  nbytes = (BLKSIZE > TASKBUFSIZ - headpos) ? TASKBUFSIZ - headpos : BLKSIZE;
		amt = write(fd, &t->buf[headpos], nbytes);
	}
...
static void register_files(task_t *tracker_task, const char *myalias)
{
	...
	while ((ent = readdir(dir)) != NULL) {
		...
		char *digest;
		digest = create_digest(ent->d_name);
		osp2p_writef(tracker_task->peer_fd, "HAVE %s [%s]\n", ent->d_name, digest);
		message("* Registered file: %s, digest: %s\n", ent->d_name, digest);
		messagepos = read_tracker_response(tracker_task);
		if (tracker_task->buf[messagepos] != '2')
...
task_t *start_download(task_t *tracker_task, const char *filename)
{
	...
	//*****************************************
	//Exercise 2: Fixed bug - buffer overflow
...
{
  //n_tasks++;
  //message("New Listening Task STARTED, n_tasks = %d\n", n_tasks);
	...
	t->peer_fd = fd;
	//n_tasks--;
	//message("New Listening Task ENDED, n_tasks = %d\n", n_tasks);
	return t;
}
