	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		switch (entry_oi->oi_ftype)
		  ...
		  case OSPFS_FTYPE_REG:
		    dt_type = DT_REG; break;
		  case OSPFS_FTYPE_DIR:
		    dt_type = DT_DIR; break;
		  case OSPFS_FTYPE_SYMLINK:
		    dt_type = DT_LNK; break;
		  }
		ok_so_far = filldir(dirent, od->od_name, strlen(od->od_name),
				    f_pos, od->od_ino, dt_type);

		//eprintk("ospfs_readdir:while, od->od_name: %s, f_pos: %u, ok_so_far:%d\n", od->od_name, f_pos, ok_so_far);
...
ospfs_unlink(struct inode *dirino, struct dentry *dentry)
{
	...
	for (entry_off = 0; entry_off < dir_oi->oi_size;
		...
		if (od->od_ino > 0
		    && strlen(od->od_name) == dentry->d_name.len
		    && memcmp(od->od_name, dentry->d_name.name, dentry->d_name.len) == 0){
		  //eprintk("ospfs_unlink, found file to remove, name: %s, offset: %d, inode: %u, oi_size: %u\n", od->od_name, entry_off, od->od_ino, dir_oi->oi_size);
			break;
		}
	}
...
add_block(ospfs_inode_t *oi)
{
	...
	uint32_t new_blockno = allocate_block();
	if(new_blockno == 0)
	  return -ENOSPC;
	zero_block(new_blockno);

	if(n == 0){
	  oi->oi_direct[0] = new_blockno;
	  oi->oi_size += OSPFS_BLKSIZE;
	  return 0;
	}
	int i_indir2 = indir2_index(n-1);
	int i_indir = indir_index(n-1);
	int i_dir = direct_index(n-1);

	//eprintk("add_block, i_indir2: %d, i_indir: %d, i_dir: %d\n", i_indir2, i_indir, i_dir);

	//if the current blocks requires indirect2 block
	if(i_indir2 == 0)
	  {
	    //eprintk("add_block, Requires indirect2 block\n");
	    //if the file cannot hold more data
	    if(i_indir == OSPFS_NINDIRECT-1 && i_dir == OSPFS_NINDIRECT-1){
	      free_block(new_blockno);
	      return -EIO;
	    }

	    if(i_dir == OSPFS_NINDIRECT-1)
	      {
		uint32_t new_indir_blockno = allocate_block();
		if(new_indir_blockno == 0)
		  {
		    free_block(new_blockno);
		    return -ENOSPC;
		  }
		zero_block(new_indir_blockno);

		uint32_t *indir2_block = ospfs_block(oi->oi_indirect2) + 4*(i_indir+1);
		*indir2_block = new_indir_blockno;
		uint32_t *indir_block = ospfs_block(new_indir_blockno);
		*indir_block = new_blockno;
	      }
	    else
	      {
		uint32_t *indir2_block = ospfs_block(oi->oi_indirect2) + 4*(i_indir);
		uint32_t *indir_block = ospfs_block(*indir2_block) + 4*(i_dir+1);
		*indir_block = new_blockno;
	      }
	  }
	//if the current blocks do not require indirect2 block
	else if(i_indir2 == -1)
	  {
	    //if the current blocks require indirect block
	    if(i_indir == 0)
	      {
		//eprintk("add_block, Requires direct and indirect blocks only\n");
		if(i_dir == OSPFS_NINDIRECT-1)
		  {
		    uint32_t new_indir2_blockno = allocate_block();
		    //if there is no free block for the indir2 block, 
		    //free new_block and return -ENOSPC;
		    if(new_indir2_blockno == 0)
		      {
			free_block(new_blockno);
			return -ENOSPC;
		      }
		    zero_block(new_indir2_blockno);

		    uint32_t new_indir_blockno = allocate_block();
		    //if there is no free block for the indir block, 
		    //free new_block and return -ENOSPC;
		    if(new_indir_blockno == 0)
		      {
			free_block(new_blockno);
			free_block(new_indir2_blockno);
			return -ENOSPC;
		      }
		    zero_block(new_indir_blockno);

			oi->oi_indirect2 = new_indir2_blockno;
			uint32_t *indir2_block = ospfs_block(new_indir2_blockno);
			*indir2_block = new_indir_blockno;
			uint32_t *indir_block = ospfs_block(new_indir_blockno);
			*indir_block = new_blockno;
		  }
		  ...
		  {
		    uint32_t *indir_block = ospfs_block(oi->oi_indirect) + 4*(i_dir+1);
		    *indir_block = new_blockno;
		  }
	      }
	    //if the current blocks do not require indirect block
	    else if(i_indir == -1)
	      {
		//eprintk("add_block, Requires direct blocks only\n");
		if(i_dir == OSPFS_NDIRECT-1)
		  {
		    uint32_t new_indir_blockno = allocate_block();
		    //if there is no free block for the indir block, 
		    //free new_block and return -ENOSPC;
			...
			oi->oi_indirect = new_indir_blockno;
			uint32_t *indir_block = ospfs_block(new_indir_blockno);
		  ...
		  oi->oi_direct[i_dir+1] = new_blockno;
    	      }
	    else
	      return -EIO;
	  }
	...
	uint32_t blk_offset = oi->oi_size % OSPFS_BLKSIZE;
	if(blk_offset == 0)
	  oi->oi_size += OSPFS_BLKSIZE;
	else
	  oi->oi_size = oi->oi_size - blk_offset + 2 * OSPFS_BLKSIZE;

	//eprintk("add_block, new size after adding a block: %u\n", oi->oi_size);
	return 0;
}
...
remove_block(ospfs_inode_t *oi)
{
	...
	//eprintk("remove_block, i_indir2: %d, i_indir: %d, i_dir: %d\n", i_indir2, i_indir, i_dir);

	uint32_t offset = (n-1) * OSPFS_BLKSIZE;
	uint32_t d_blockno = ospfs_inode_blockno(oi, offset);
	free_block(d_blockno);

	//if the file has indirect2 block right now: 
	if(i_indir2 == 0){
	  ...
	  if(i_dir == 0){
	    ...
	    *indir2_block = 0;
	    ...
	    uint32_t *indir2_block = ospfs_block(oi->oi_indirect2) + 4*i_indir;
	    uint32_t *indir_block = ospfs_block(*indir2_block) + 4*i_dir;
	    *indir_block = 0;
	  }
	}
	//if the file does not have indirect2 block right now:
	else if(i_indir2 == -1){
	  
	  //if the file has a indirect block
	  if(i_indir == 0){
	    
	    if(i_dir == OSPFS_NDIRECT){
	      free_block(d_blockno);
	      ...
	      free_block(d_blockno);
	      uint32_t *indir_block = ospfs_block(oi->oi_indirect) + 4*i_dir;
	      *indir_block = 0;
	    }
	  }
	  //if the file has no indirect block (direct block only)
	  else if(i_indir == -1){
	    oi->oi_direct[n-1] = 0;
	    free_block(d_blockno);
	  }
	...
	uint32_t shrink_nbytes =  oi->oi_size % OSPFS_BLKSIZE;
	if(shrink_nbytes == 0)
	  shrink_nbytes = OSPFS_BLKSIZE;
	oi->oi_size -= shrink_nbytes;
	return 0;
}
...
change_size(ospfs_inode_t *oi, uint32_t new_size)
{
	...
	//eprintk("Change_size, old_size: %u, new_size: %u\n", old_size, new_size);

	while (ospfs_size2nblocks(oi->oi_size) < ospfs_size2nblocks(new_size)) {
	        /* EXERCISE: Your code here */
	  //eprintk("Change_size, add a block\n");
	  r = add_block(oi);
	  if(r == -ENOSPC || r == -EIO)
	    {
	      new_size = old_size;
	...
	while (ospfs_size2nblocks(oi->oi_size) > ospfs_size2nblocks(new_size)) {
	        /* EXERCISE: Your code here */
	  //eprintk("Change_size, remove a block\n");
	  r = remove_block(oi);
	  if(r == -EIO)
	    break;
	}
	...
	//eprintk("Change_size, after change, new_size = %u\n", oi->oi_size);
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	/* EXERCISE: Your code here */
	uint32_t append = filp->f_flags & O_APPEND;
	int first_block = 1;
	uint32_t n1;
	if(append)
	  *f_pos = oi->oi_size;
	...
	////eprintk("Going to write, append: %u, count: %u, *f_pos: %u, oi_size: %u\n",
	//append, count, *f_pos, oi->oi_size);
	...
	while (amount < count && retval >= 0) {
		...
		*f_pos += n;
		...
		//eprintk("%u bytes wrote, count = %u, *f_pos = %u\n", n, count, *f_pos);
	}
...
find_direntry(ospfs_inode_t *dir_oi, const char *name, int namelen)
{
	...
	for (off = 0; off < dir_oi->oi_size; off += OSPFS_DIRENTRY_SIZE) {
		...
		if (od->od_ino
		  ...
		  //eprintk("find_direntry, found direntry with name: %s, name_len: %d, offset: %d\n", name, namelen, off);
			return od;
		}
	}
	return 0;
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	od->od_ino = 0;
	od->od_name[0] = 0;
	//eprintk("create_blank_direntry, new direntry offset:%d, oi_size:%u\n", off, dir_oi->oi_size);
	return od;
}
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
	/* EXERCISE: Your code here. */
  ospfs_direntry_t *link_entry;
  ospfs_inode_t *dir_oi = ospfs_inode(dir->i_ino);
  
  if(find_direntry(dir_oi, dst_dentry->d_name.name, dst_dentry->d_name.len))
    return -EEXIST;

  link_entry = create_blank_direntry(dir_oi);
  if(IS_ERR(link_entry))
    return PTR_ERR(link_entry);

  memcpy(link_entry->od_name, src_dentry->d_name.name, src_dentry->d_name.len);
  link_entry->od_ino = src_dentry->d_inode->i_ino;
...
find_free_inode(void)
{
  uint32_t i = 1;
  ospfs_inode_t *oi;
  while(i < ospfs_super->os_ninodes)
    {
      oi = ospfs_inode(i);
      if(oi->oi_nlink == 0)
	return i;
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
  ...
  uint32_t block_num;
  ospfs_inode_t *dir_oi = ospfs_inode(dir->i_ino);
  uint32_t entry_ino = find_free_inode();
	/* EXERCISE: Your code here. */
	...
	//eprintk("ospfs_create, new file created with iname: %s, inode: %u\n", new_entry->od_name, entry_ino);
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	/* EXERCISE: Your code here. */
	unsigned name_len = strlen(symname);
	...
	symlink_oi->oi_size = name_len;
	memcpy(symlink_oi->oi_symlink, symname, name_len);
	symlink_oi->oi_symlink[name_len] = 0;
