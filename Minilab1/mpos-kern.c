...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_WAIT: {
		...
		if (p <= 0 || p >= NPROCS || p == current->p_pid
		  ...
		  {
		    proc_array[p].p_waiting = current;
...
do_fork(process_t *parent)
{
  ...
  for(i = 1; i<NPROCS; i++)
	...
	{
	  memcpy(&(proc_array[i].p_registers), &(parent->p_registers), sizeof(registers_t));
	  proc_array[i].p_registers.reg_eax = 0;
	  proc_array[i].p_state = parent->p_state;
...
copy_stack(process_t *dest, process_t *src)
{
	...
	dest->p_registers.reg_esp = (uint32_t)memcpy((void*)dest_stack_bottom, (const void*)src_stack_bottom, src_stack_top - src_stack_bottom);
